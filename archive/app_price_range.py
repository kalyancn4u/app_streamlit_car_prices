...
with st.container(border=True):
    c1, c2 = st.columns(2)
    with c1:
        make_options = list(makes_models.keys())
        default_make = make_options.index("MARUTI") if "MARUTI" in make_options else 0
        make = st.selectbox("🏷️ Make / Brand", options=make_options,
    ...
    with c3:
        fuel = st.selectbox("⛽ Fuel", categorical["fuel"]["options"])
    with c4:
        transmission = st.selectbox("⚙️ Transmission",
                                    categorical["transmission"]["options"])
    with c5:
        seller = st.selectbox("🤝 Seller type", categorical["seller"]["options"])
    with c6:
        seats = st.selectbox("💺 Seats", categorical["seats"]["options"])
