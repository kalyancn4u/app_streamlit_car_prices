...
│
├── data/
│   └── cars24-car-price-cleaned-new.csv.gz  # 🗃 Dataset, gzip-compressed (training only)
│
...
| File / folder | Purpose |
| :------------ | :------ |
| `data/…cleaned-new.csv.gz` | The dataset, **gzip-compressed** (~81% smaller; pandas reads it directly). Needed **only** for (re)training — the running app doesn't read it. |
...
[`data/cars24-car-price-cleaned-new.csv.gz`](data/cars24-car-price-cleaned-new.csv.gz)
— a **gzip-compressed CSV** (~290 KB, about **81 % smaller** than the 1.5 MB raw
file). This section explains what that file is, how to inspect it, the Excel
"it opens when I click it" trap for CSVs, and the storage formats you can choose from.

### It's a normal CSV, just compressed — and pandas reads it directly

A **CSV** ("comma-separated values") is a plain-text table: one row per line,
columns separated by commas. This one is **gzip-compressed** (`.csv.gz`) purely to
save space. You never unzip it yourself: `pandas.read_csv("…csv.gz")` infers the
`.gz` extension and decompresses it in memory, so `train_model.py` reads it with
**no code change** from when it was a raw `.csv` (pandas does the same for `.bz2`,
`.xz` and `.zip`).

### Inspecting it (it won't open in Excel — and that's expected)

Because the extension is `.gz`, double-clicking hands the file to an **archive
tool** (7-Zip / WinRAR), not Excel — it's a compressed archive, not a spreadsheet.
You don't need to extract it; to peek inside:

| Goal | Do this |
| :--- | :------ |
| Read a few rows | `zcat data/…csv.gz \| head` (Git Bash), or `pd.read_csv("data/…csv.gz", nrows=5)` |
| Confirm what it really is | `file data/…csv.gz` (reports the true type from the file's bytes, not its name) |
| See true extensions in Explorer | File Explorer → **View → File name extensions** (Windows hides them, so `data.csv` may really be `data.csv.gz`) |

### The Excel trap (for when you *do* decompress to a raw `.csv`)

If you ever unzip this to a plain `.csv`, Windows opens it in **Excel** on
double-click — handy for a peek, but Excel silently rewrites data: it strips
leading zeros (`007` → `7`), turns long numbers into scientific notation
(`9.19E+09`), reads text like `3-4` as **dates**, and can change the encoding on save.

> ⚠️ **Never double-click a data CSV, edit it, and hit Save** — you can corrupt it
> without any warning. Use a text editor, `head`, or pandas to look at raw data.
...
- **Best size/speed balance for a git repo:** **`CSV + gzip`** — 5× smaller than
  ...
  pandas reads it directly. **This is what this project (and its Flask and MLOps
  siblings) ship.**
